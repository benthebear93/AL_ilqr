...
class TestObjective(unittest.TestCase):
    def test_objective_and_gradients(self):
        ...
        # define costs
        def ot(x, u, w=None):
            return np.dot(x, x) + 0.1 * np.dot(u, u)

        def oT(x, u, w=None):
            return 10.0 * np.dot(x, x)
        ...
        # test inputs
        x1 = np.ones(num_state)
        u1 = np.ones(num_action)
        w1 = np.zeros(num_parameter)

        X = [x1 for _ in range(T)]
        U = [u1 for _ in range(T - 1)] + [np.zeros(0)]
        ...
        self.assertTrue(np.allclose(grad_x_ct, 2.0 * x1, atol=1e-8))
        self.assertTrue(np.allclose(grad_u_ct, 0.2 * u1, atol=1e-8))

        val_cT = cT.evaluate(x1, np.zeros(0), np.zeros(0))
        grad_x_cT = cT.gradient_state(x1, np.zeros(0), np.zeros(0))
        self.assertAlmostEqual(val_cT, oT(x1, np.zeros(0)), places=8)
        self.assertTrue(np.allclose(grad_x_cT, 20.0 * x1, atol=1e-8))
